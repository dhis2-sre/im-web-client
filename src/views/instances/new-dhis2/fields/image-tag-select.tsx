...
import { SearchableSingleSelect, Option } from '../../../../components/searchable-single-select-2.tsx'
...
function useImageTagField() {
    const { input } = useField<string>(IMAGE_TAG_FIELD_NAME, { subscription: { initial: true } })
...
function useCheckImageExists(repository) {
    ...
    const checkImageExists = useCallback(
        async ({ tag, repository }) => {
            console.log('> { tag, repository }', { tag, repository })
